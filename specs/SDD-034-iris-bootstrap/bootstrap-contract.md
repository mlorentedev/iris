...
**Substrate constraint:** dev runs **Local-process** per [[adr-002-orchestrator-architecture]] line 183 — motor as native binary, workers + NATS in containers. Motor does NOT run in Docker during development (fast iteration loop via `air` hot-reload).
...
2. `docker compose -f compose.dev.yml up -d nats worker-hermes`
...
| 4 | Worker hermes container running | `docker compose ps -q worker-hermes \| xargs docker inspect --format '{{.State.Status}}' \| grep -q '^running$'` |
...
**Approach:** code is the contract (Go interfaces + structs as primary source of truth) — same shape as agent_crew's production-validated model, but with the gaps they left open closed from day 1. See [[agent-crew-analysis|agent_crew analysis]] § 6, 7, 9 for the prior-art audit.
...
| Console HTMX URLs | `internal/console/handlers/*.go` | Route registrations + HTML templates (`embed.FS`) | **BETA** — UI evolves, URLs stable within `/v1`. Per [[adr-001-frontend-htmx-go]] |
...
| Multi-tenant discriminator | every persisted model | `team_id` column + middleware injection per [[adr-002-orchestrator-architecture]] | **FROZEN** |
...
| Frontend types manually mirror backend | iris uses HTMX server-side rendering — there is no separate client type system to drift. Side-effect of [[adr-001-frontend-htmx-go]] |
...
`hermes | opencode | ollama` — **NEVER `claude`** (Anthropic ToS, per [[adr-002-orchestrator-architecture]] § "Why no claude provider"). The motor under `internal/` must contain ZERO provider-specific code. Provider-specific logic lives only in capability sub-interface implementations (e.g., `internal/runtime/hermes/`).
...
| 4 | **Dev environment** — `Makefile` targets (`dev / install-tools / smoke-test / docker-build / api-docs`) + `.air.toml` for Go hot-reload + `compose.dev.yml` with services `nats` (port 4223) + `worker-hermes` (stub Python image, healthcheck only for now) + `make smoke-test` script implementing the 5 checks from § 2 | `make install-tools && make dev & sleep 5; make smoke-test; kill %1` |
...
- Architecture ADR: [[adr-002-orchestrator-architecture]] (renumbered from kubelab/adr-038 on 2026-05-19)
- License ADR: [[adr-003-license-apache-2]] (renumbered from kubelab/adr-040 on 2026-05-19)
- Frontend ADR: [[adr-001-frontend-htmx-go]] (renumbered from kubelab/adr-035 on 2026-05-19)
- Positioning ADR (added 2026-05-21): [[adr-004-positioning]]
- Architectural boundary ADR (added 2026-05-21): [[adr-005-architectural-boundary]]
