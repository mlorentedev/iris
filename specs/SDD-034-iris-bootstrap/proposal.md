...
3. **Dev environment ergonomic:** `make dev` levanta NATS:4223 + worker-hermes stub + motor via air (hot-reload); `make smoke-test` ejecuta los 5 checks del bootstrap-contract § 2 y exit 0 iff substrate funciona end-to-end.
...
- **Cualquier código de los SDD-034 posteriores.** NATS envelope (SDD-034b), K8s runtime adapter agent-sandbox wrapping (SDD-034c), Python workers Hermes adapter (SDD-034d), HTMX console (SDD-034e), E2E test (SDD-034f), kubelab manifests (SDD-034g) — todos cero código en este PR. Solo stubs `internal/{api,scheduler,runtime,protocol,post-actions,console,db}/` con paquetes vacíos para que `go build ./...` compile.
...
- **OIDC auth + RBAC + vault MCP integration + Hermes runtime.** ADR-002 los declara pero v0 no implementa. Auth provider stub en `internal/api/` solo expone interface `Provider` con impl `noop`. Vault MCP integration es runtime concern (agents leen vault) — fuera del motor bootstrap.
...
- [ ] **AC4: Dev environment + smoke test green.** `make install-tools && make dev` brings up `nats:4223 + worker-hermes-stub + motor (air hot-reload)`; `make smoke-test` ejecuta los 5 checks del bootstrap-contract § 2 y exit 0. (matches `sdd-034a-f4-dev-environment`)
...
- ADR: [[adr-002-orchestrator-architecture]] (canonical architecture — was kubelab/adr-038, renumbered 2026-05-19)
- ADR: [[adr-003-license-apache-2]] (license decision — was kubelab/adr-040)
- ADR: [[adr-004-positioning]] (FDE-grade EU platform teams Series A-C K8s on-prem) — 2026-05-21
- ADR: [[adr-005-architectural-boundary]] (vault-as-SSOT + CORE/EDGE + Combo 8 + DP-1..5 + EQ-1 + métricas) — 2026-05-21
