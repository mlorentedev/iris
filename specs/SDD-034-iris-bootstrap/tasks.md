...
- [ ] **Task 4: Dev environment** (`features.json` id: `sdd-034a-f4-dev-environment`) — Makefile (`dev/install-tools/smoke-test/docker-build/api-docs`) + `.air.toml` + `compose.dev.yml` (`nats:4223` + `worker-hermes` stub) + smoke-test script implementing 5 checks. Verification: `make install-tools && make dev & sleep 5; make smoke-test; kill %1`.
