...
> **Amends:** [adr-006-pivot-ai-native-engineering-studio](./adr-006-pivot-ai-native-engineering-studio.md) §4 (un-defers the Go motor — see "Amendment to ADR-006 §4").
...
This **un-defers the Go motor** (ADR-006 §4), ratified consciously on 2026-06-13: the team chose "full motor + NATS" over the minimal alternatives because copying agent_crew's architecture is *implementing already-extracted patterns*, not greenfield scaffolding — which neutralises the solitary-scaffold-stall risk ADR-006 §4 was guarding against.
...
- **[adr-006-pivot-ai-native-engineering-studio](./adr-006-pivot-ai-native-engineering-studio.md) §4** — had deferred the Go motor "until real coordination friction appears." Amended here.
...
State verified on disk (Phase A): `~/Projects/iris/` and `~/Projects/imagingsuite/` **do not exist yet** (repo creation is the act that un-defers, per ADR-006 post-decision signal); board `mlorentedev/knowledge` has the SDD-034a-i stream live (#74, #55-59, #75-77).
...
4. **Human sign-off** — Manu approves in the **HTMX operator console** (authenticated by kubelab's **Authelia forwardAuth** — reuses the existing identity gate, no new auth) or via channel/GitHub → merge. **Never auto-merge** (hard rule; the variance control of ADR-006's "expert sign-off").
...
**Net:** copy the architecture (free) + pi (MIT) + the gate they lack. "Inherit their blueprints, not their house to renovate." Apache 2.0 clean; zero AGPL code in any release.

## Amendment to ADR-006 §4

ADR-006 §4 deferred the Go motor "until real coordination friction appears (scaffolding is commodity)." This ADR **un-defers it**, on these grounds, ratified by the maintainer 2026-06-13:
- Copying agent_crew's architecture is **implementing already-extracted patterns**, not greenfield scaffolding — the specific risk §4 guarded against (solitary-scaffold stall) does not apply.
- A **real forcing function now exists** (imagesensortool), which the revealed-preference evidence in ADR-006 said was the missing precondition.
- **pi (MIT)** collapses the worker + inference cost to a dependency, not a build.

ADR-006 §4's deferral of the **resident loop-mode supervisor** (the autonomous "what next / when to stop" layer) **still stands** — that is ADR-008 / #72, gated on the human-in-the-loop v0 proving out first.
...
| License | Apache 2.0; copy architecture only, never AGPL code |
...
- **Differentiation vs agent_crew is concrete**: verification gate, sovereign/cheap inference (pi-ai over NaN/Ollama, the helmcode-nan thesis), rich n8n integration, Apache-clean.
...
- [adr-002-orchestrator-architecture](./adr-002-orchestrator-architecture.md) · [adr-006-pivot-ai-native-engineering-studio](./adr-006-pivot-ai-native-engineering-studio.md) · [adr-003-license-apache-2](./adr-003-license-apache-2.md) · [adr-001-frontend-htmx-go](./adr-001-frontend-htmx-go.md)
